...
-- REMOVE ALL OLD RETURNS AND OLD FLY SCRIPT ABOVE THIS LINE

-----------------------------------------------------
-- FLY SCRIPT WITH GUI INTEGRATION
-----------------------------------------------------

getgenv().FlyToggle = getgenv().FlyToggle or false
getgenv().FlySpeed = getgenv().FlySpeed or 60
...
local function updateMoveDirection()
    ...
    moveDirection = (x==0 and z==0) and Vector3.new() or Vector3.new(x,0,z).Unit
end

UserInputService.InputBegan:Connect(function(input, gp)
    if gp then return end

    -- ONLY toggle if GUI Fly is enabled
    if input.KeyCode == Enum.KeyCode.X then
        if not getgenv().FlyToggle then return end
...
RunService.RenderStepped:Connect(function()
    if flying and bodyVelocity and bodyGyro then
        ...
        bodyVelocity.Velocity =
            cam.CFrame:VectorToWorldSpace(moveDirection) * speed +
